...
interface HostingPlanControlsProps {
  ...
  onBillingChange:
    | Dispatch<SetStateAction<BillingCycle>>
    | ((cycle: BillingCycle) => void);
  dataCenter: string;
  onDataCenterChange:
...
const HostingPlanControls = ({
  ...
  const shouldShowDivider =
    showDivider ?? ((isBlur || isPlain) && !hideDataCenter);
  const availableCycles =
    cycles ?? (Object.keys(cycleLabels) as BillingCycle[]);

  const containerClass = isBlur
    ? `flex w-full items-center justify-center gap-6 px-6 py-3 rounded-2xl lg:w-auto ${className}`
  ...
  const toggleWrapperClass = isBlur
    ? "flex flex-wrap justify-center items-center rounded-full px-1 py-1"
    : isPlain
      ? "flex flex-wrap justify-center items-center rounded-full px-1.5 py-1"
  ...
  const labelClass = isBlur ? "text-sm font-bold" : "text-sm font-medium";
  ...
  return (
    <div className={containerClass} style={containerStyle}>
        ...
        {availableCycles.map((cycle) => {
          ...
          return (
            <button
              ...
              className="px-4 py-1 rounded-full font-medium text-xs sm:px-5 sm:py-1.5 sm:text-sm transition-colors duration-200"
              style={{
                borderRadius: "9999px",
      ...
      {shouldShowDivider && <div className="h-6 w-px" style={dividerStyle} />}
