...
interface KnowledgeArticleProps {
  articleData: KnowledgeArticlePageData;
}

const KnowledgeArticle: React.FC<KnowledgeArticleProps> = ({ articleData }) => {
  const [isLoaded, setIsLoaded] = useState(false);
  const [activeSection, setActiveSection] = useState<string>(
    articleData.sections[0]?.id || "",
  );
  ...
  const getAllSectionIds = (sections: KnowledgeArticleSection[]): string[] => {
    let ids: string[] = [];
  ...
  const itemIds = getAllSectionIds(articleData.sections);
  ...
  const renderBlock = (block: KnowledgeArticleContentBlock, index: number) => {
    ...
    if (block.type === "image") {
      return (
        <figure key={index} className="my-4 sm:my-6 space-y-2">
  ...
  const renderSection = (section: KnowledgeArticleSection) => (
    <div
      key={section.id}
  ...
  const sidebarItems = articleData.sections.map((section) => ({
    id: section.id,
    label: section.title,
  }));
          ...
          <div className="lg:col-span-2 lg:pr-4">
            ...
            >
              <div className="text-xs text-[rgb(var(--policy-text-muted))]">
                ...
                <span>{articleData.breadcrumbCategory}</span>
              </div>
              <ContentDescription
              ...
              <div className="space-y-4 sm:space-y-6 text-xs leading-relaxed text-[rgb(var(--policy-text-white))]">
                {articleData.intro.map((paragraph, index) => (
                  <p key={index}>{paragraph}</p>
                ))}
              </div>
              {articleData.sections.map(renderSection)}
            </motion.div>
          </div>
